...
标明字段的取值范围，设置的方式有三种
1.第一种是采用枚举的形式。
例如：`allowableValues="first, second, third"`
...
参数的数据类型，如果我们设置了这个属性，将被覆盖掉通过内省获得的参数的数据类型。并且这个数据类型可以是基本数据类型，也可以是类的名字。如果是基本数据类型，为了防止抛出`java.lang.NumberFormatException: For input string: ""`的错误
1.我们可以采用配置`example`属性一起使用
2.我们可以通过升级`swagger-annotations`和`swagger-models`的版本来避免，升级到`1.5.21`版本即可。

### required属性

表示的是当前字段是否是必须的，默认是`false`。

### position属性

已过时的方法，代表是属性在文档中的位置排序。

### hidden属性

表示的是是否隐藏当前字段，默认是`false`。
...
过时方法，在`Swagger1.5.19`版本之后，采用`accessMode`注解代替。

### accessMode属性

属性的数据模式。使用的是一个枚举`AccessMode`的值，其中包括`AUTO`、`READ_ONLY`、`READ_WRITE`。

### reference属性

指定了属性的类型引用，如果设置了当前属性，会覆盖任何其他的元数据(`不常使用`)。
...
该属性用于进行额外的描述。是一个可选项的数组组成。

## 总结

`@ApiModelProperty`注解主要是对于类的内部的一个属性的描述。