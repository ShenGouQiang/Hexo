...
原本打算，将`resize`单独抽出来，当做一篇文章进行讲解的。但是后来想了想，还是决定放在`put`操作进行讲解。因为在`resize`主要是在`put`中进行的。接下来，我们看下resize的源码原理。在后面，我会通过动画的形式，来展现出`resize`的执行步骤。但是关于`resize`的具体分析，依然需要文字进行描述。
